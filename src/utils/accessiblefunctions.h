...
#define FUNC_PRESS(classobj)     QStringList actionNames() const override{\
    ...
    void doAction(const QString &actionName) override{\
        ...
        {\
            QPointF localPos = classobj->rect().center();\
...
#define FUNC_COMBOXPOPUP(classobj)     QStringList actionNames() const override{\
        if(!classobj->isEnabled())\
            return QStringList();\
        return QStringList() << "popUp";}\
    void doAction(const QString &actionName) override{\
        if(actionName == "popUp")\
        {\
            classobj->showPopup();\
...
#define FUNC_RECT(classobj) QRect rect() const override{\
        if (!classobj->isVisible())\
            return QRect();\
        return QRect(classobj->mapToGlobal(classobj->rect().topLeft()),classobj->size())/*classobj->geometry()*/;\
    }\
...
#define FUNC_TEXT(classname,accessiblename) QString Accessible##classname::text(QAccessible::Text t) const{\
        ...
        default:\
            return QAccessibleWidget::text(t)/*return QString()*/;\
        }\
    }\
...
#define SET_COMBOX_ACCESSIBLE(classname,accessiblename)                        SET_COMBOX_ACCESSIBLE_DESCRIPTION(classname,QAccessible::ComboBox,accessiblename,"")\
    FUNC_TEXT(classname,accessiblename)
/************************************************************************************************/
