...
void RotationTool::mousePressEvent(QGraphicsSceneMouseEvent *event, MainGraphicsScene *scene)
{
    DrawTool::mousePressEvent(event, scene);
    if ( event->button() != Qt::LeftButton ) return;
...
void RotationTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, MainGraphicsScene *scene)
{
    DrawTool::mouseMoveEvent(event, scene);
    QList<QGraphicsItem *> items = scene->selectedItems();
    if ( items.count() == 1 ) {
        AbstractShape *item = qgraphicsitem_cast<AbstractShape *>(items.first());
        if ( item != nullptr  && nDragHandle != Handle_None && selectMode == rotate ) {
...
void RotationTool::mouseReleaseEvent(QGraphicsSceneMouseEvent *event, MainGraphicsScene *scene)
{
    DrawTool::mouseReleaseEvent(event, scene);
    if ( event->button() != Qt::LeftButton ) return;
